...
func RedisScan(info *common.HostInfo) (tmperr error) {
	...
	for _, pass := range common.Passwords {
		...
		if flag == true && err == nil {
			...
			if time.Now().Unix()-starttime > (int64(len(common.Passwords)) * common.TcpTimeout) {
				return err
			}
		}
	}
...
func RedisConn(info *common.HostInfo, pass string) (flag bool, err error) {
	...
	defer conn.Close()
	err = conn.SetReadDeadline(time.Now().Add(time.Duration(common.TcpTimeout) * time.Second))
	if err != nil {
		return flag, err
	}
