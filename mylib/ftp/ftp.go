...
func Dial(addr string, options ...DialOption) (*ServerConn, error) {
	...
	if dialFunc == nil {
		//fmt.Println("[debug] dialFunc is nil!")
		...
		if do.tlsConfig != nil && !do.explicitTLS {
			//fmt.Println("[debug] dialFunc set to tls dial!")
			...
			//fmt.Println("[debug] dialFunc set to tcp dial!")
			proxyDial := common.GetProxyDialer()
			if d, ok := proxyDial.(net.Dialer); ok {
				//do.dialer = d
				d.Timeout = DefaultDialTimeout
				dialFunc = func(network, address string) (net.Conn, error) {
					return do.dialer.DialContext(ctx, network, addr)
				}
			} else if d, ok := proxyDial.(proxy.Dialer); ok {
				dialFunc = func(network, address string) (net.Conn, error) {
...
func (c *ServerConn) openDataConn() (net.Conn, error) {
	...
	host = strings.Split(c.finalTargetAddr, ":")[0]
	//fmt.Println("[debug] openDataConn start, host=", host)

	if err != nil {
		//fmt.Println("[debug] getDataConnPort err:", err)
		return nil, err
	}
	...
	if c.options.dialFunc != nil {
		//fmt.Println("[debug] dialFunc is not null!", "target:", addr)
		if d, ok := proxyDial.(net.Dialer); ok {
			//fmt.Println("[debug] dialFunc set to net dialer!", "target:", addr)
			d.Timeout = DefaultDialTimeout
			return c.options.dialer.Dial("tcp", addr)
		} else if d, ok := proxyDial.(proxy.Dialer); ok {
			//fmt.Println("[debug] dialFunc set to proxy!", "target:", addr)
	...
	//fmt.Println("[debug] ", "target:", addr)

	if d, ok := proxyDial.(net.Dialer); ok {
		//fmt.Println("[debug] dialFunc set to net dialer 2!", "target:", addr)
		d.Timeout = DefaultDialTimeout
		return c.options.dialer.Dial("tcp", addr)
	} else if d, ok := proxyDial.(proxy.Dialer); ok {
		//fmt.Println("[debug] dialFunc set to proxy2!", "target:", addr)
		return d.Dial("tcp", addr)
	}
	return c.options.dialer.Dial("tcp", addr)
}
